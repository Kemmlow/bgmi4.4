...
inline SDK::ASTExtraPlayerCharacter *GetKnoxyHyperTarget(SDK::FVector &otp)
{
    ...
    if (!c || !ct || !ct->PlayerCameraManager)
        return nullptr;
    ...
    for (auto a : as)
    {
        if (!a || a == (SDK::AActor *)c || isObjectInvalid(a))
            continue;
        if (!a->IsA(SDK::ASTExtraPlayerCharacter::StaticClass()))
            continue;
        auto e = (SDK::ASTExtraPlayerCharacter *)a;
        if (e->TeamID == c->TeamID)
            continue;
        ...
        for (const char *bn : TargetBonesFallback)
        {
            ...
            if (ct->LineOfSightTo(e, cl, false))
            {
                vbp = b;
                v = true;
                break;
    ...
    inline void ApplyNuclearTrueDamage(SDK::ASTExtraBaseCharacter *c)
    {
        ...
        if (ct)
        {
            if (ct->AntiCheatManagerComp)
            {
                ...
                ct->bShouldReportAntiCheat = false;
            }
        }
    }
}
...
inline void xShootBulletInner(uintptr_t W, SDK::FVector SL, SDK::FRotator SR, int SID)
{
    ...
    if (knoxy::BulletTrack)
    {
        ...
        if (t)
        {
            // God-Level Direct Manifestation: Spawn bullet directly at target bone
            return ShootBulletInner_Orig(W, tp, SR, SID);
        }
    }
    return ShootBulletInner_Orig(W, SL, SR, SID);
}
